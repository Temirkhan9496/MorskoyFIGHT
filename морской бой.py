...
class Board:
    ...
    def __str__(self):
        res = ""
        res += "|  | " + " | ".join([str(x+1) for x in range(self.size)]) + "|\n"
...
def create_board(size):
    ...
    for l in [3, 2, 2, 1, 1, 1, 1]:
        while True:
            ...
            if attempts > 2000:
                return None
            ship = Ship(Dot(random.randint(0, size), random.randint(0, size)), l, random.randint(0, 1))
            try:
                board.add_ship(ship)
                break
